...
constexpr std::size_t kMaximumSerializedSourceBuffers = 1U << 16U;
constexpr std::size_t kMaximumSerializedSourceSamples = 1U << 24U;

void SetStateError(std::string* error, const std::string& message) {
    if (error != nullptr) {
        *error = message;
    }
}
...
nlohmann::json Source::CaptureState() const {
    const auto captureBuffer = [](const Buffer& buffer) {
        return nlohmann::json{
            {"physical_address", buffer.physical_address},
            {"length", buffer.length},
            {"adpcm_ps", buffer.adpcm_ps},
            {"adpcm_yn", buffer.adpcm_yn},
            {"adpcm_dirty", buffer.adpcm_dirty},
            {"is_looping", buffer.is_looping},
            {"buffer_id", buffer.buffer_id},
            {"mono_or_stereo", static_cast<uint32_t>(buffer.mono_or_stereo)},
            {"format", static_cast<uint32_t>(buffer.format)},
            {"from_queue", buffer.from_queue},
            {"play_position", buffer.play_position},
            {"has_played", buffer.has_played},
        };
    };
    const auto captureSourceState = [&](const SourceState& sourceState) {
        nlohmann::json queue = nlohmann::json::array();
        auto remaining = sourceState.input_queue;
        while (!remaining.empty()) {
            queue.push_back(captureBuffer(remaining.top()));
            remaining.pop();
        }
        return nlohmann::json{
            {"enabled", sourceState.enabled},
            {"sync_count", sourceState.sync_count},
            {"gain", sourceState.gain},
            {"input_queue", std::move(queue)},
            {"mono_or_stereo",
             static_cast<uint32_t>(sourceState.mono_or_stereo)},
            {"format", static_cast<uint32_t>(sourceState.format)},
            {"current_sample_number", sourceState.current_sample_number},
            {"current_buffer_physical_address",
             sourceState.current_buffer_physical_address},
            {"current_buffer", sourceState.current_buffer.CopyRemaining()},
            {"buffer_update", sourceState.buffer_update},
            {"last_buffer_id", sourceState.last_buffer_id},
            {"current_buffer_id", sourceState.current_buffer_id},
            {"adpcm_coeffs", sourceState.adpcm_coeffs},
            {"adpcm_state",
             {{"yn1", sourceState.adpcm_state.yn1},
              {"yn2", sourceState.adpcm_state.yn2}}},
            {"rate_multiplier", sourceState.rate_multiplier},
            {"interpolation_mode",
             static_cast<uint32_t>(sourceState.interpolation_mode)},
            {"interpolation_state",
             {{"xn1", sourceState.interp_state.xn1},
              {"xn2", sourceState.interp_state.xn2},
              {"fractional_position", sourceState.interp_state.fposition}}},
            {"filters", sourceState.filters.CaptureState()},
        };
    };

    return {
        {"schema", "azahar_hle_source_v1"},
        {"source_id", source_id},
        {"current_frame", current_frame},
        {"backup_frame", backup_frame},
        {"state", captureSourceState(state)},
        {"backup_state", captureSourceState(backup_state)},
    };
}

bool Source::RestoreState(const nlohmann::json& snapshot,
                          std::string* error) {
    try {
        if (!snapshot.is_object() ||
            snapshot.value("schema", std::string{}) !=
                "azahar_hle_source_v1" ||
            snapshot.at("source_id").get<std::size_t>() != source_id) {
            SetStateError(error, "invalid HLE source state identity");
            return false;
        }

        const auto decodeMonoOrStereo = [&](const nlohmann::json& value,
                                            MonoOrStereo& output) {
            const auto raw = value.get<uint32_t>();
            if (raw != static_cast<uint32_t>(MonoOrStereo::Mono) &&
                raw != static_cast<uint32_t>(MonoOrStereo::Stereo)) {
                SetStateError(error,
                              "invalid mono/stereo value in HLE source state");
                return false;
            }
            output = static_cast<MonoOrStereo>(raw);
            return true;
        };
        const auto decodeFormat = [&](const nlohmann::json& value,
                                      Format& output) {
            const auto raw = value.get<uint32_t>();
            if (raw > static_cast<uint32_t>(Format::ADPCM)) {
                SetStateError(error, "invalid format in HLE source state");
                return false;
            }
            output = static_cast<Format>(raw);
            return true;
        };
        const auto decodeInterpolation = [&](const nlohmann::json& value,
                                             InterpolationMode& output) {
            const auto raw = value.get<uint32_t>();
            if (raw > static_cast<uint32_t>(InterpolationMode::None)) {
                SetStateError(error,
                              "invalid interpolation mode in HLE source state");
                return false;
            }
            output = static_cast<InterpolationMode>(raw);
            return true;
        };
        const auto decodeBuffer = [&](const nlohmann::json& encoded,
                                      Buffer& buffer) {
            buffer.physical_address = encoded.at("physical_address").get<PAddr>();
            buffer.length = encoded.at("length").get<u32>();
            buffer.adpcm_ps = encoded.at("adpcm_ps").get<u8>();
            buffer.adpcm_yn =
                encoded.at("adpcm_yn").get<std::array<u16, 2>>();
            buffer.adpcm_dirty = encoded.at("adpcm_dirty").get<bool>();
            buffer.is_looping = encoded.at("is_looping").get<bool>();
            buffer.buffer_id = encoded.at("buffer_id").get<u16>();
            if (!decodeMonoOrStereo(encoded.at("mono_or_stereo"),
                                    buffer.mono_or_stereo) ||
                !decodeFormat(encoded.at("format"), buffer.format)) {
                return false;
            }
            buffer.from_queue = encoded.at("from_queue").get<bool>();
            buffer.play_position = encoded.at("play_position").get<u32>();
            buffer.has_played = encoded.at("has_played").get<bool>();
            return true;
        };
        const auto decodeSourceState = [&](const nlohmann::json& encoded,
                                           SourceState& output) {
            SourceState restored{};
            restored.enabled = encoded.at("enabled").get<bool>();
            restored.sync_count = encoded.at("sync_count").get<u16>();
            restored.gain = encoded.at("gain")
                                .get<std::array<std::array<float, 4>, 3>>();
            const auto& queue = encoded.at("input_queue");
            const auto& currentBuffer = encoded.at("current_buffer");
            if (!queue.is_array() ||
                queue.size() > kMaximumSerializedSourceBuffers ||
                !currentBuffer.is_array() ||
                currentBuffer.size() > kMaximumSerializedSourceSamples) {
                SetStateError(error, "HLE source state exceeds size limits");
                return false;
            }
            for (const auto& encodedBuffer : queue) {
                Buffer buffer{};
                if (!decodeBuffer(encodedBuffer, buffer)) {
                    return false;
                }
                restored.input_queue.push(buffer);
            }
            if (!decodeMonoOrStereo(encoded.at("mono_or_stereo"),
                                    restored.mono_or_stereo) ||
                !decodeFormat(encoded.at("format"), restored.format)) {
                return false;
            }
            restored.current_sample_number =
                encoded.at("current_sample_number").get<u32>();
            restored.current_buffer_physical_address =
                encoded.at("current_buffer_physical_address").get<PAddr>();
            restored.current_buffer =
                currentBuffer.get<AudioCore::StereoBuffer16>();
            restored.buffer_update = encoded.at("buffer_update").get<bool>();
            restored.last_buffer_id = encoded.at("last_buffer_id").get<u16>();
            restored.current_buffer_id =
                encoded.at("current_buffer_id").get<u16>();
            restored.adpcm_coeffs =
                encoded.at("adpcm_coeffs").get<std::array<s16, 16>>();
            restored.adpcm_state.yn1 =
                encoded.at("adpcm_state").at("yn1").get<s16>();
            restored.adpcm_state.yn2 =
                encoded.at("adpcm_state").at("yn2").get<s16>();
            restored.rate_multiplier =
                encoded.at("rate_multiplier").get<float>();
            if (!(restored.rate_multiplier > 0.0F) ||
                !std::isfinite(restored.rate_multiplier) ||
                !decodeInterpolation(encoded.at("interpolation_mode"),
                                     restored.interpolation_mode)) {
                SetStateError(error,
                              "invalid sample rate in HLE source state");
                return false;
            }
            const auto& interpolation = encoded.at("interpolation_state");
            restored.interp_state.xn1 =
                interpolation.at("xn1").get<std::array<s16, 2>>();
            restored.interp_state.xn2 =
                interpolation.at("xn2").get<std::array<s16, 2>>();
            restored.interp_state.fposition =
                interpolation.at("fractional_position").get<u64>();
            if (!restored.filters.RestoreState(encoded.at("filters"), error)) {
                return false;
            }
        ...
        SourceState restoredState{};
        SourceState restoredBackupState{};
        if (!decodeSourceState(snapshot.at("state"), restoredState) ||
            !decodeSourceState(snapshot.at("backup_state"),
                               restoredBackupState)) {
            return false;
        }
        const auto restoredCurrentFrame =
            snapshot.at("current_frame").get<StereoFrame16>();
        const auto restoredBackupFrame =
            snapshot.at("backup_frame").get<StereoFrame16>();

        state = std::move(restoredState);
        backup_state = std::move(restoredBackupState);
        current_frame = restoredCurrentFrame;
        backup_frame = restoredBackupFrame;
        return true;
    } catch (const std::exception& exception) {
        SetStateError(error,
                      std::string("invalid HLE source state: ") +
                          exception.what());
        return false;
    }
}
...
void Source::SetMemoryResolver(PhysicalMemoryResolver resolver) {
    memory_resolver = std::move(resolver);
}
...
void Source::ParseConfig(SourceConfiguration::Configuration& config,
    ...
    if (config.partial_embedded_buffer_dirty) {
        ...
        const u8* const memory =
            memory_resolver
                ? memory_resolver(state.current_buffer_physical_address &
                                  0xFFFFFFFC)
                : nullptr;
        ...
        if (memory) {
            ...
            if (valid) {
                ...
                if (state.current_buffer.size() < state.current_sample_number) {
                    ...
                    state.current_buffer.Consume(state.current_sample_number);
                }
            }
        }
...
bool Source::DequeueBuffer() {
    ...
    const u8* const memory =
        memory_resolver
            ? memory_resolver(buf.physical_address & 0xFFFFFFFC)
            : nullptr;
    if (memory) {
