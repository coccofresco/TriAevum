...
nlohmann::json Mixers::CaptureState() const {
    const auto captureMixerState = [](const MixerState& mixer) {
        return nlohmann::json{
            {"intermediate_mixer_volume", mixer.intermediate_mixer_volume},
            {"aux_bus_enable", mixer.aux_bus_enable},
            {"intermediate_mix_buffer", mixer.intermediate_mix_buffer},
            {"output_format", static_cast<uint32_t>(mixer.output_format)},
        };
    };
    return {
        {"schema", "azahar_hle_mixers_v1"},
        {"current_frame", current_frame},
        {"backup_frame", backup_frame},
        {"state", captureMixerState(state)},
        {"backup_state", captureMixerState(backup_state)},
    };
}

bool Mixers::RestoreState(const nlohmann::json& snapshot,
                          std::string* error) {
    try {
        if (!snapshot.is_object() ||
            snapshot.value("schema", std::string{}) !=
                "azahar_hle_mixers_v1") {
            SetStateError(error, "invalid HLE mixer state schema");
            return false;
        }
        const auto decodeMixerState = [&](const nlohmann::json& encoded,
                                          MixerState& output) {
            MixerState restored{};
            restored.intermediate_mixer_volume =
                encoded.at("intermediate_mixer_volume")
                    .get<std::array<float, 3>>();
            restored.aux_bus_enable =
                encoded.at("aux_bus_enable").get<std::array<bool, 2>>();
            restored.intermediate_mix_buffer =
                encoded.at("intermediate_mix_buffer")
                    .get<std::array<QuadFrame32, 3>>();
            const auto outputFormat = encoded.at("output_format").get<uint32_t>();
            if (outputFormat > static_cast<uint32_t>(OutputFormat::Surround)) {
                SetStateError(error, "invalid output format in HLE mixer state");
                return false;
            }
            restored.output_format = static_cast<OutputFormat>(outputFormat);
            output = std::move(restored);
            return true;
        };

        MixerState restoredState{};
        MixerState restoredBackupState{};
        if (!decodeMixerState(snapshot.at("state"), restoredState) ||
            !decodeMixerState(snapshot.at("backup_state"),
                              restoredBackupState)) {
            return false;
        }
        ...
        SetStateError(error,
                      std::string("invalid HLE mixer state: ") +
                          exception.what());
        return false;
    }
}
