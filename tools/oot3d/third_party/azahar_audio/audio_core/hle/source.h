...
public:
    ...
    using PhysicalMemoryResolver = std::function<const u8*(PAddr)>;

    /// Sets the physical-memory resolver used for source sample buffers.
    void SetMemoryResolver(PhysicalMemoryResolver resolver);
...
private:
    ...
    PhysicalMemoryResolver memory_resolver;
    ...
    private:
        ...
        void serialize(Archive& ar, const unsigned int) {
            ...
            // Preserve the donor archive's deque representation, but not its
            // per-sample allocation policy during playback.
            std::deque<std::array<s16, 2>> serialized;
            if constexpr (Archive::is_saving::value) {
                serialized.assign(current_buffer.begin(), current_buffer.end());
            }
            ar & serialized;
            if constexpr (Archive::is_loading::value) {
                current_buffer = AudioCore::StereoBuffer16(serialized.begin(), serialized.end());
            }
            ar & buffer_update;
