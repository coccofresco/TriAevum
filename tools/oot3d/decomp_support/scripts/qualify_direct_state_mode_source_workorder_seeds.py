#!/usr/bin/env python3
"""Qualify state/mode source workorder probe rows before split promotion."""
...
from qualify_direct_source_subregion_seeds import NEAR_CATEGORIES, classify_seed
from sweep_direct_source_subregion_ranges import float_value, int_value, list_value, repo_path


ROOT = Path(__file__).resolve().parents[1]
DEFAULT_PROBE = ROOT / "analysis" / "direct_state_mode_source_workorder_probe.json"
DEFAULT_DISASSEMBLY = ROOT / "ghidra_export" / "disassembly.txt"
DEFAULT_OUT_JSON = ROOT / "analysis" / "direct_state_mode_source_workorder_seed_qualification.json"
DEFAULT_OUT_CSV = ROOT / "analysis" / "direct_state_mode_source_workorder_seed_qualification.csv"
DEFAULT_OUT_MD = ROOT / "analysis" / "direct_state_mode_source_workorder_seed_qualification.md"
...
def target_rows_for(disasm: list[dict[str, Any]], handler_range: str) -> list[dict[str, Any]]:
    ...
    return [
        row
        for row in disasm
        if start <= int(str(row.get("addr", "0")), 16) < end
    ]
...
def qualify_rows(probe: dict[str, Any], disasm: list[dict[str, Any]]) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for row in list_value(probe.get("rows", [])):
        if not isinstance(row, dict):
            continue
        handler_rows = target_rows_for(disasm, str(row.get("handler_range", "")))
        target_start = int_value(row.get("target_window_start_offset"))
        compare_count = int_value(row.get("compare_instruction_count"))
        target_ops = [
            str(item.get("op", ""))
            for item in handler_rows[target_start : target_start + compare_count]
        ]
        compiled_all_ops = compiled_ops_for(row)
        compiled_skip = int_value(row.get("compiled_skip"))
        compiled_ops = compiled_all_ops[compiled_skip : compiled_skip + compare_count]
        ...
        rows.append(
            {
                ...
                "handler_range": row.get("handler_range", ""),
...
def build_report(args: argparse.Namespace) -> dict[str, Any]:
    ...
    rows = qualify_rows(probe, disasm)
    ...
    summary = {
        ...
        "best_seed_workorder": best.get("workorder_id", ""),
        "best_seed_handler": best.get("handler", ""),
        "best_seed_source": (
            f"{best.get('best_source_function', '')}:{best.get('best_source_range', '')}"
            if best
            else ""
        ),
        ...
        "next_gate": "Use qualified seeds for smaller branch/body isolation; keep risk rows out of promotion.",
    }
    return {
        "format": "oot3d_direct_state_mode_source_workorder_seed_qualification_v1",
        "inputs": {
...
def write_markdown(path: Path, data: dict[str, Any]) -> None:
    ...
    lines = [
        "# Direct State/Mode Source Workorder Seed Qualification",
        "",
        "This report qualifies near/exact handler/source probe rows before they are used as split seeds.",
        "",
        "## Summary",
        "",
        "| Metric | Value |",
    ...
    lines.extend(
        [
            ...
            "## Top Seeds",
            "",
            "| Class | Workorder | Handler | Source | Target start | Skip | Compare | LCS | Run | Generic | Reason |",
            "| --- | --- | --- | --- | --- | ---: | ---: | ---: | --- | ---: | --- |",
        ]
    )
    for row in data["rows"]:
        ...
        lines.append(
            f"| `{row.get('seed_class', '')}` | `{row.get('workorder_id', '')}` | "
            f"`{row.get('handler', '')}` | `{source}` | `{row.get('target_start_addr', '')}` | "
...
def main() -> int:
    ...
    print(
        "direct state/mode source workorder seed qualification: "
        f"{summary['qualified_seed_rows']} qualified, {summary['risk_rows']} risk, "
        f"best {summary['best_seed_workorder']} {summary['best_seed_class']}"
    )
