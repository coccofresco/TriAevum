...
KANKYO_ENVIRONMENT_ARCHIVE_MANIFEST = "oot3d_kankyo_environment_export_manifest.json"


def pack_kankyo_environment_export_manifest(
    export_manifest_path: Path,
    output_path: Path,
    *,
    name: str = "OOT3D Kankyo Environment Candidates",
    author: str = "local",
    version: str = "0.1.0",
) -> Path:
    if not export_manifest_path.is_file():
        raise ParseError(f"{export_manifest_path}: kankyo environment export manifest not found")
    ...
    root_manifest = {
        ...
        "author": author,
        "version": version,
        "description": "OOT3D kankyo sky/environment resources for offline Shipwright asset testing.",
    ...
    with zipfile.ZipFile(output_path, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        ...
        archive.writestr(KANKYO_ENVIRONMENT_ARCHIVE_MANIFEST, json.dumps(export_manifest, indent=2) + "\n")
        written_paths.update({ROOT_ARCHIVE_MANIFEST, KANKYO_ENVIRONMENT_ARCHIVE_MANIFEST})

        for resource in converted_export_resources(export_manifest):
            archive_path = normalize_resource_path(resource["path"])
            if not archive_path:
                raise ValueError("converted kankyo resource has an empty archive path")
...
def audit_kankyo_environment_export_package(
    export_manifest_path: Path,
    archive_path: Path,
    ...
    *,
    ...
    try:
        with zipfile.ZipFile(archive_path, "r") as archive:
            ...
            has_export_manifest = KANKYO_ENVIRONMENT_ARCHIVE_MANIFEST in archive_name_set
            root_manifest_error = read_json_error(archive, ROOT_ARCHIVE_MANIFEST) if has_root_manifest else None
            export_manifest_error = (
                read_json_error(archive, KANKYO_ENVIRONMENT_ARCHIVE_MANIFEST)
            ...
            if has_export_manifest and export_manifest_error is None:
                archived_export_manifest = json.loads(
                    archive.read(KANKYO_ENVIRONMENT_ARCHIVE_MANIFEST).decode("utf-8")
                )
            ...
            extra_resource_entries = sorted(
                ...
                and path not in {ROOT_ARCHIVE_MANIFEST, KANKYO_ENVIRONMENT_ARCHIVE_MANIFEST}
            )
    ...
    issue_counts = {
        ...
        "missing_kankyo_environment_export_manifest": 0 if has_export_manifest else 1,
        "invalid_kankyo_environment_export_manifest_json": 1 if export_manifest_error else 0,
        "kankyo_environment_export_manifest_mismatch": (
    ...
    audit: dict[str, object] = {
        "format": "oot3d_kankyo_environment_export_package_audit_v1",
        "export_manifest": str(export_manifest_path),
        "archive": str(archive_path),
        "archive_entry_count": len(archive_names),
        "has_manifest": has_root_manifest,
        "has_kankyo_environment_export_manifest": has_export_manifest,
        ...
        "kankyo_environment_export_manifest_error": export_manifest_error,
        ...
        "fallback_policy": [
            ...
            "It does not route Shipwright environment rendering to OOT3D kankyo resources by itself.",
            "N64 environment rendering remains fallback until explicit runtime routing is implemented.",
        ],
    }
