            ...
            with (
                ...
                arguments = {
                    ...
                    "toolchain": backend.WholeAotPluginToolchain(
                        ...
                        target_triple=platforms.WINDOWS.triple, profile=platforms.WINDOWS.profile,
                    ),
                    "shard_count": 2,
                    "jobs": 2,
                }
    ...
    def test_linux_target_links_hidden_symbol_shared_object(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                ...
                self.assertEqual(built["target"], platforms.LINUX.triple)
                self.assertEqual(Path(built["plugin"]).name, platforms.LINUX.plugin)
                compile_command, link_command = commands
                for flag in ("-fPIC", "-fvisibility=hidden", "-c"):
                    self.assertIn(flag, compile_command)
                for flag in ("-shared", "-fuse-ld=lld", "-Wl,-Bsymbolic", "-Wl,--exclude-libs,ALL", "-static-libstdc++"):
                ...
                with self.assertRaisesRegex(backend.WholeAotPluginError, "sysroot"):
                    backend.build_whole_aot_plugin(
                        ...
                        toolchain=backend.WholeAotPluginToolchain(
                            ...
                            nlohmann_include=nlohmann, target_triple=platforms.LINUX.triple,
                            profile=platforms.LINUX.profile, sysroot=root))
