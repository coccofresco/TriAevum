...
def title_layout(*, runtime: Path, native_module: Path, plugin_manifest: Path,
    ...
    # Receipts predating the "target" field are Windows builds.
    platform = platforms.for_triple(str(build.get("target", platforms.WINDOWS.triple)))
    ...
    plugin = plugin_manifest.parent / platform.plugin
    record = artifact(plugin, f"titles/{name}/{platform.plugin}")
    ...
    catalog = {"format": FORMAT, "install_model": MODEL,
               ...
               "native_module": artifact(native_module, platform.native_module_path),
               "titles": [{"recipe": name, "inputs": recipe["inputs"], "abi_version": 2,
                           "target": platform.triple, "plugin": record,
