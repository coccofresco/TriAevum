...
class WholeAotObjectCacheTests(unittest.TestCase):
    ...
    def build(self, **overrides):
        arguments = {
            ...
            "toolchain": NativeToolchain(self.compiler, self.archiver, target_triple=release_platform.WINDOWS.target, profile=release_platform.WINDOWS.profile),
            "jobs": 2,
        ...
        with self.assertRaisesRegex(WholeAotObjectError, "unsupported.*profile"):
            self.build(
                toolchain=NativeToolchain(
                    self.compiler,
                    self.archiver,
                    target_triple=release_platform.WINDOWS.target,
    ...
    def test_linux_target_uses_elf_recipes(self) -> None:
        ...
        for command in compile_commands:
            self.assertIn("--target=" + release_platform.LINUX.target, command)
        ...
        with self.assertRaisesRegex(WholeAotObjectError, "profile"):
            self.build(toolchain=NativeToolchain(
                self.compiler, self.archiver,
                target_triple=release_platform.LINUX.target, profile=release_platform.WINDOWS.profile))
