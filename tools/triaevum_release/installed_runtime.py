...
def validate_installed_runtime(
    ...
    plugin = resolve_reference(str(runtime.get("plugin") or (executable.parent / platforms.host().plugin)), title)
    ...
    elif plugin.resolve() != (executable.parent / platforms.host().plugin).resolve():
