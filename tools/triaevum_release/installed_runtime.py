...
def validate_installed_runtime(
    ...
    if "renderer_cache" in runtime:
        option = "--renderer-cache-directory"
        expected = resolve_reference(runtime["renderer_cache"]["path"], title)
        if arguments.count(option) != 1:
            raise ValueError("Installed renderer cache routing is missing or duplicated")
        index = arguments.index(option) + 1
        if index >= len(arguments) or Path(arguments[index]).resolve() != expected:
            raise ValueError("Launch profile points to another renderer cache")
        # Mutable/disposable cache, not a release artifact. Missing or corrupt
        # entries are regenerated by the renderer; never hash or require them.
    return profile
