...
void GfxWindowBackendSDL2::SetFullscreenImpl(bool on, bool call_callback) {
    ...
    const bool windowedFullscreen =
        Ship::Context::GetRawInstance()
            ->GetConsoleVariables()
            ->GetInteger(CVAR_SDL_WINDOWED_FULLSCREEN, 0) != 0;
    if (on && (windowedFullscreen ||
               !mExclusiveDisplayModeConfigured)) {
    ...
    mFullScreen = on;
#else
    if (SDL_SetWindowFullscreen(mWnd, on ? (windowedFullscreen
...
#endif
    mExclusiveDisplayModeConfigured = false;
...
static void EnsureWindowPositionIsVisible(int32_t& posX, int32_t& posY, uint32_t width, uint32_t height) {
    if (SDL_WINDOWPOS_ISUNDEFINED(posX) || SDL_WINDOWPOS_ISCENTERED(posX) ||
        SDL_WINDOWPOS_ISUNDEFINED(posY) || SDL_WINDOWPOS_ISCENTERED(posY)) {
        return;
    }

    const int64_t windowRight = static_cast<int64_t>(posX) + width;
    const int64_t windowBottom = static_cast<int64_t>(posY) + height;
    ...
    for (int displayIndex = 0; displayIndex < displayCount; ++displayIndex) {
        ...
        if (posX < displayRight && windowRight > bounds.x && posY < displayBottom && windowBottom > bounds.y) {
            return;
        }
    }

    SDL_Rect primaryBounds;
    if (SDL_GetDisplayUsableBounds(0, &primaryBounds) != 0 && SDL_GetDisplayBounds(0, &primaryBounds) != 0) {
        posX = 100;
        posY = 100;
        return;
    }

    const int32_t visibleWidth = std::min<int32_t>(static_cast<int32_t>(width), primaryBounds.w);
    const int32_t visibleHeight = std::min<int32_t>(static_cast<int32_t>(height), primaryBounds.h);
    posX = primaryBounds.x + std::max<int32_t>(0, (primaryBounds.w - visibleWidth) / 2);
    posY = primaryBounds.y + std::max<int32_t>(0, (primaryBounds.h - visibleHeight) / 2);
}
...
void GfxWindowBackendSDL2::Init(const char* gameName, const char* gfxApiName, bool startFullScreen, uint32_t width,
...
#else
    const bool use_opengl = !mUsesVulkan;
#endif
...
#elif defined(__SWITCH__)
    // Mesa/nouveau in devkitPro exposes the core features required by the
    // canonical PICA shader path through an OpenGL 4.3 context.
    SDL_GL_SetAttribute(SDL_GL_CONTEXT_PROFILE_MASK, SDL_GL_CONTEXT_PROFILE_CORE);
    SDL_GL_SetAttribute(SDL_GL_CONTEXT_MAJOR_VERSION, 4);
    ...
    mWindowWidth = docked ? 1920 : 1280;
    mWindowHeight = docked ? 1080 : 720;
    ...
    if (mWnd == nullptr) {
        throw std::runtime_error(std::string("SDL window creation failed for ") + gfxApiName + ": " +
                                 SDL_GetError());
    }
#ifdef _WIN32
    ...
    if (mUsesVulkan) {
        ...
        window_impl.Backend = WindowBackend::FAST3D_SDL_OOT3D_VULKAN;
#endif
    } else if (use_opengl) {
#ifndef __SWITCH__
        ...
        if (mCtx == nullptr) {
            throw std::runtime_error(std::string("SDL OpenGL context creation failed: ") + SDL_GetError());
        }

        SDL_GL_MakeCurrent(mWnd, mCtx);
#ifdef __SWITCH__
        if (!gladLoadGLLoader(reinterpret_cast<GLADloadproc>(SDL_GL_GetProcAddress))) {
            throw std::runtime_error(std::string("Switch OpenGL loader initialization failed: ") + SDL_GetError());
        }
#endif
...
void GfxWindowBackendSDL2::RequestFocus() {
    if (mWnd == nullptr) {
        return;
    }
    SDL_RaiseWindow(mWnd);
    SDL_SetWindowInputFocus(mWnd);
}
...
void GfxWindowBackendSDL2::SwapBuffersBegin() {
    ...
    if (!mUsesVulkan) {
        // Zero is the explicit uncapped contract used by the renderer
        // throughput benchmark. Keep the ordinary 60 Hz legacy default for
        // interactive callers that never select a target.
        if (mTargetFps != 0U) {
...
bool GfxWindowBackendSDL2::IsWindowedFullscreen() const {
    if (mWnd == nullptr) {
        return false;
    }
...
bool GfxWindowBackendSDL2::SetExclusiveFullscreenDisplayMode(
    uint32_t width, uint32_t height) {
    if (mWnd == nullptr || width == 0U || height == 0U) {
        return false;
    }
    int displayInUse = SDL_GetWindowDisplayIndex(mWnd);
    if (displayInUse < 0) {
        displayInUse = 0;
    }
    SDL_DisplayMode requested{};
    requested.format = SDL_PIXELFORMAT_UNKNOWN;
    requested.w = static_cast<int>(width);
    requested.h = static_cast<int>(height);
    SDL_DisplayMode closest{};
    if (SDL_GetClosestDisplayMode(
            displayInUse, &requested, &closest) == nullptr) {
        SPDLOG_ERROR(
            "No SDL exclusive fullscreen mode matches {}x{}: {}",
            width, height, SDL_GetError());
        return false;
    }
    if (SDL_SetWindowDisplayMode(mWnd, &closest) < 0) {
        SPDLOG_ERROR(
            "Failed to select SDL exclusive fullscreen mode {}x{}: {}",
            closest.w, closest.h, SDL_GetError());
        return false;
    }
    mExclusiveDisplayModeConfigured = true;
    return true;
}

void* GfxWindowBackendSDL2::GetNativeWindow() const {
    return mWnd;
}

bool GfxWindowBackendSDL2::UsesVulkan() const {
    return mUsesVulkan;
}
} // namespace Fast
#endif
