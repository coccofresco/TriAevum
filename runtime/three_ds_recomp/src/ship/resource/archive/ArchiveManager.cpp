...
std::string ResolveArchivePath(const std::filesystem::path& path) {
            ...
            if (std::filesystem::is_directory(archivePath)) {
                ...
                for (const auto& p : std::filesystem::directory_iterator(archivePath)) {
                    if (StringHelper::IEquals(p.path().extension().string(), ".otr") ||
                        ...
                        fileList.push_back(ResolveArchivePath(p));
                        hasAssetFiles = true;
                    }
                }
                ...
                SPDLOG_WARN("The archive at path {} does not exist",
