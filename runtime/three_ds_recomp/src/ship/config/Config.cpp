...
bool Config::SetBlock(const std::string& key, nlohmann::json block) {
    nlohmann::json gjson = mFlattenedJson.unflatten();
    if (!SetNestedConfigBlock(
            gjson, key, std::move(block))) {
        return false;
    }
...
bool Config::Save() {
    if (mPath == "None" || mPath.empty()) {
        return true;
    }
    mNestedJson = mFlattenedJson.unflatten();
    const auto result = WriteConfigFileAtomically(
        fs::path(mPath), mNestedJson.dump(4));
    if (!result.Success) {
        SPDLOG_ERROR(
            "Unable to atomically save config file {}: {}",
            mPath, result.Error);
    }
