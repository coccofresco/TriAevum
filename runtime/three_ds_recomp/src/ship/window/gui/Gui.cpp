...
void Gui::DrawMenu() {
    ...
    if (!OwnsApplicationShortcuts() && (ImGui::IsKeyPressed(TOGGLE_BTN, false) || ImGui::IsKeyPressed(ImGuiKey_Escape, false) ||
...
bool Gui::GetAnyGuiWindowVisible() {
    return std::any_of(mGuiWindows.begin(), mGuiWindows.end(), [](const auto& entry) {
