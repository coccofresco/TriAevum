...
TEST(ContextPathResolution, PreservesAbsoluteApplicationPaths) {
    const fs::path absolute =
        (fs::temp_directory_path() / "oot3d_graphics.json")
            .lexically_normal();
    EXPECT_EQ(
        fs::path(Ship::Context::GetPathRelativeToAppDirectory(
                     absolute.string()))
            .lexically_normal(),
        absolute);
}
