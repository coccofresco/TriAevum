...
TEST(ArchiveManager, LoadFileUsesMostRecentlyAddedArchiveForDuplicatePath) {
    auto base = LoadedArchive("ram://base", { { "scenes/test/CollisionHeader", "base" } });
    auto override = LoadedArchive("ram://override", { { "scenes/test/CollisionHeader", "override" } });
    Ship::ArchiveManager am;
    am.AddArchive(base);
    am.AddArchive(override);

    auto file = am.LoadFile("scenes/test/CollisionHeader");
    ASSERT_NE(file, nullptr);
    std::string content(file->Buffer->begin(), file->Buffer->end());
    EXPECT_EQ(content, "override");
    ASSERT_NE(am.GetArchiveFromFile("scenes/test/CollisionHeader"), nullptr);
    EXPECT_EQ(am.GetArchiveFromFile("scenes/test/CollisionHeader")->GetPath(), "ram://override");
}
