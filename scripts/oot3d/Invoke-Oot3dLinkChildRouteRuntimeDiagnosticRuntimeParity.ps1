...
if ([string]::IsNullOrWhiteSpace($PackageRoot)) {
    $PackageRoot = Join-Path $characterRoot "route_runtime_diagnostic_package"
}
if ([string]::IsNullOrWhiteSpace($OutputRoot)) {
    $OutputRoot = Join-Path $characterRoot "route_runtime_diagnostic_runtime_parity"
}
...
function Assert-Condition([bool]$Condition, [string]$Message) {
    if (-not $Condition) {
        throw "OOT3D Link child route/runtime diagnostic runtime parity failed: $Message"
    }
}
...
Require-Path $PackageRoot "Route/runtime diagnostic package root"

$packageSummaryPath = Join-Path $PackageRoot "link_child_route_runtime_diagnostic_package_summary.json"
$packageAuditPath = Join-Path $PackageRoot "link_child_route_runtime_diagnostic_package_audit.json"
$trackBatchManifestPath = Join-Path $PackageRoot "link_child_route_runtime_diagnostic_track_batch_manifest.json"
$archivePath = Join-Path $PackageRoot "oot3d_link_child_route_runtime_diagnostic_candidates.o2r"
$summaryOutput = Join-Path $OutputRoot "link_child_route_runtime_diagnostic_runtime_parity_summary.json"
$auditRoot = Join-Path $OutputRoot "audits"
$glbRoot = Join-Path $OutputRoot "glb"

Require-Path $packageSummaryPath "Route/runtime diagnostic package summary"
Require-Path $packageAuditPath "Route/runtime diagnostic package audit"
Require-Path $trackBatchManifestPath "Route/runtime diagnostic track batch manifest"
Require-Path $archivePath "Route/runtime diagnostic package archive"
New-Item -ItemType Directory -Force -Path $OutputRoot, $auditRoot, $glbRoot | Out-Null
...
if ($Verify) {
    ...
    Assert-Condition ([int]$packageSummary.exported -eq 8) "expected 8 packaged route/runtime tracks"
    Assert-Condition ([int]$packageSummary.failed -eq 0) "expected no package export failures"
    Assert-Condition ([int]$packageSummary.route_runtime_capture_counts.accepted_route_count -eq 0) "expected no accepted route/runtime routes"
    Assert-Condition ([int]$packageSummary.route_runtime_capture_counts.installed_runtime_dump_valid_count -eq 0) "expected no accepted installed route/runtime dumps"
    Assert-Condition ([int]$packageSummary.package_audit_counts.issue_counts.total -eq 0) "expected package audit issue count 0"
    Assert-Condition ($trackBatchManifest.status -eq "route_runtime_diagnostic_track_batch_ready") "expected route/runtime track batch ready"
    Assert-Condition ([int]$trackBatchManifest.exported -eq 8) "expected 8 route/runtime track batch records"
...
foreach ($record in @($trackBatchManifest.records)) {
    ...
    Require-Path $trackPath "Packaged route/runtime diagnostic track"
    ...
    Require-Path $auditOutput "Route/runtime diagnostic runtime parity audit"
    Require-Path $runtimeGlbOutput "Route/runtime diagnostic runtime GLB"
    ...
    $records += [ordered]@{
        ...
        source_contract_kind = $record.source_contract_kind
        frontier_class = $record.frontier_class
        reference_envelope_status = $record.reference_envelope_status
        route_context_class = $record.route_context_class
        route_trigger = $record.route_trigger
        capture_status = $record.capture_status
        route_acceptance_gate = $record.route_acceptance_gate
        ...
        runtime_acceptance_status = "offline_runtime_skinned_pose_parity_valid_pending_exact_route_or_installed_draw_capture"
    }
}
...
$summary = [ordered]@{
    format = "oot3d_link_child_route_runtime_diagnostic_runtime_parity_summary_v1"
    status = $status
    runtime_acceptance_status = if ($status -eq "offline_runtime_skinned_pose_parity_valid") {
        "offline_runtime_skinned_pose_parity_valid_pending_exact_route_or_installed_draw_capture"
    } else {
        "offline_runtime_skinned_pose_parity_invalid"
    }
    ...
    records = $records
    next_gate = "Prove exact route, accepted sibling route, or capture/diff installed runtime draw output before promotion."
}
$summary | ConvertTo-Json -Depth 12 | Set-Content -LiteralPath $summaryOutput -Encoding UTF8

if ($Verify) {
    Assert-Condition ($summary.status -eq "offline_runtime_skinned_pose_parity_valid") "expected offline runtime/skinned pose parity valid"
    Assert-Condition ($summary.runtime_acceptance_status -eq "offline_runtime_skinned_pose_parity_valid_pending_exact_route_or_installed_draw_capture") "expected runtime acceptance to remain pending route/capture"
    Assert-Condition ($summary.exported -eq 8) "expected eight runtime parity records"
    Assert-Condition ($summary.failed -eq 0) "expected zero failed runtime parity records"
    Assert-Condition ($summary.counts.runtime_track_count -eq 8) "expected eight runtime tracks"
    Assert-Condition ($summary.counts.valid_runtime_track_count -eq 8) "expected eight valid runtime tracks"
    Assert-Condition ($summary.counts.runtime_primitive_count -eq 96) "expected 96 selected runtime primitives"
    Assert-Condition ($summary.counts.runtime_skinned_primitive_count -eq 80) "expected 80 selected skinned primitives"
    Assert-Condition ($summary.counts.runtime_rigid_primitive_count -eq 16) "expected 16 selected rigid primitives"
    Assert-Condition ($summary.counts.issue_count -eq 0) "expected zero runtime parity issues"
    Assert-Condition ($summary.counts.animated_position_row_count -eq $summary.counts.finite_position_row_count) "expected all animated positions finite"
    Assert-Condition ($summary.counts.animated_position_row_count -eq $summary.counts.finite_normal_row_count) "expected all animated normals finite"
}

Write-Host "OOT3D Link child route/runtime diagnostic runtime parity: $summaryOutput"
Write-Host "status=$($summary.status) exported=$($summary.exported) issues=$($summary.counts.issue_count)"
