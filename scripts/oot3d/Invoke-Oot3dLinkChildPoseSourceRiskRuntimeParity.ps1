...
if ([string]::IsNullOrWhiteSpace($PackageRoot)) {
    $PackageRoot = Join-Path $characterRoot "pose_source_risk_diagnostic_package"
}
if ([string]::IsNullOrWhiteSpace($OutputRoot)) {
    $OutputRoot = Join-Path $characterRoot "pose_source_risk_diagnostic_runtime_parity"
}
...
function Assert-Condition([bool]$Condition, [string]$Message) {
    if (-not $Condition) {
        throw "OOT3D Link child pose/source-risk runtime parity failed: $Message"
    }
}
...
Require-Path $PackageRoot "Pose/source-risk diagnostic package root"

$packageSummaryPath = Join-Path $PackageRoot "link_child_pose_source_risk_diagnostic_package_summary.json"
$packageAuditPath = Join-Path $PackageRoot "link_child_pose_source_risk_diagnostic_package_audit.json"
$trackBatchManifestPath = Join-Path $PackageRoot "link_child_pose_source_risk_diagnostic_track_batch_manifest.json"
$archivePath = Join-Path $PackageRoot "oot3d_link_child_pose_source_risk_diagnostic_candidates.o2r"
$summaryOutput = Join-Path $OutputRoot "link_child_pose_source_risk_diagnostic_runtime_parity_summary.json"
$auditRoot = Join-Path $OutputRoot "audits"
$glbRoot = Join-Path $OutputRoot "glb"

Require-Path $packageSummaryPath "Pose/source-risk diagnostic package summary"
Require-Path $packageAuditPath "Pose/source-risk diagnostic package audit"
Require-Path $trackBatchManifestPath "Pose/source-risk diagnostic track batch manifest"
Require-Path $archivePath "Pose/source-risk diagnostic package archive"
...
if ($Verify) {
    ...
    Assert-Condition ([int]$packageSummary.exported -eq 16) "expected 16 packaged pose/source-risk tracks"
    ...
    Assert-Condition ($trackBatchManifest.status -eq "pose_source_risk_diagnostic_track_batch_ready") "expected pose/source-risk track batch ready"
    Assert-Condition ([int]$trackBatchManifest.exported -eq 16) "expected 16 pose/source-risk track batch records"
...
foreach ($record in @($trackBatchManifest.records)) {
    ...
    Require-Path $trackPath "Packaged pose/source-risk track"
    ...
    Require-Path $auditOutput "Pose/source-risk runtime parity audit"
    Require-Path $runtimeGlbOutput "Pose/source-risk runtime GLB"
...
$summary = [ordered]@{
    format = "oot3d_link_child_pose_source_risk_diagnostic_runtime_parity_summary_v1"
    status = $status
    ...
    records = $records
    next_gate = "Find alternate source, stronger route evidence, or capture/diff installed runtime draw output before promotion."
}
...
if ($Verify) {
    ...
    Assert-Condition ($summary.exported -eq 16) "expected sixteen runtime parity records"
    Assert-Condition ($summary.failed -eq 0) "expected zero failed runtime parity records"
    Assert-Condition ($summary.counts.runtime_track_count -eq 16) "expected sixteen runtime tracks"
    Assert-Condition ($summary.counts.valid_runtime_track_count -eq 16) "expected sixteen valid runtime tracks"
    Assert-Condition ($summary.counts.runtime_primitive_count -eq 192) "expected 192 selected runtime primitives"
    Assert-Condition ($summary.counts.runtime_skinned_primitive_count -eq 160) "expected 160 selected skinned primitives"
    Assert-Condition ($summary.counts.runtime_rigid_primitive_count -eq 32) "expected 32 selected rigid primitives"
...
Write-Host "OOT3D Link child pose/source-risk runtime parity: $summaryOutput"
