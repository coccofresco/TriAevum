param(
    ...
    [string]$ArchiveName = "oot3d_link_child_route_runtime_diagnostic_candidates.o2r",
    [string]$Name = "OOT3D Link Child Route Runtime Diagnostic Candidates",
    [string]$Author = "local",
...
function Assert-Condition([bool]$Condition, [string]$Message) {
    if (-not $Condition) {
        throw "OOT3D Link child route/runtime diagnostic package verification failed: $Message"
    }
}
...
$routeMatrixSummaryPath = Join-Path $characterRoot "route_runtime_capture_matrix\link_child_route_runtime_capture_matrix_summary.json"
$routeMatrixCsv = Join-Path $characterRoot "route_runtime_capture_matrix\link_child_route_runtime_capture_matrix.csv"
Require-Path $frontierCsv "Link child semantic blocked frontier CSV"
Require-Path $routeMatrixSummaryPath "Link child route/runtime capture matrix summary"
Require-Path $routeMatrixCsv "Link child route/runtime capture matrix CSV"

$outputRoot = Join-Path $characterRoot "route_runtime_diagnostic_package"
$generatedTrackRoot = Join-Path $outputRoot "generated_tracks"
$diagnosticPlanOutput = Join-Path $outputRoot "link_child_route_runtime_diagnostic_plan.json"
$diagnosticPlanCsvOutput = Join-Path $outputRoot "link_child_route_runtime_diagnostic_plan.csv"
$diagnosticTrackManifestOutput = Join-Path $outputRoot "link_child_route_runtime_diagnostic_track_manifest.json"
$diagnosticTrackCsvOutput = Join-Path $outputRoot "link_child_route_runtime_diagnostic_track_manifest.csv"
$diagnosticBuildSummaryOutput = Join-Path $outputRoot "link_child_route_runtime_diagnostic_build_summary.json"
$manifestOutput = Join-Path $outputRoot "link_child_route_runtime_diagnostic_track_batch_manifest.json"
$archiveOutput = Join-Path $outputRoot $ArchiveName
$packageAuditOutput = Join-Path $outputRoot "link_child_route_runtime_diagnostic_package_audit.json"
$summaryOutput = Join-Path $outputRoot "link_child_route_runtime_diagnostic_package_summary.json"
...
Require-Path $diagnosticBuildSummaryOutput "Route/runtime diagnostic build summary"
Require-Path $diagnosticTrackManifestOutput "Route/runtime diagnostic track manifest"

$frontierRowsByName = @{}
foreach ($row in @(Import-Csv -LiteralPath $frontierCsv | Where-Object { $_.frontier_gate -eq "route_or_runtime_capture" })) {
    $frontierRowsByName[[string]$row.n64_name] = $row
}
$routeRowsByName = @{}
foreach ($row in @(Import-Csv -LiteralPath $routeMatrixCsv)) {
    $routeRowsByName[[string]$row.n64_name] = $row
}
...
$routeMatrixSummary = Get-Content -LiteralPath $routeMatrixSummaryPath -Raw | ConvertFrom-Json
...
foreach ($trackRow in @($diagnosticTrackManifest.rows)) {
    ...
    Assert-Condition ($routeRowsByName.ContainsKey($n64Name)) "missing route/runtime matrix row for $n64Name"
    ...
    $routeRow = $routeRowsByName[$n64Name]
    ...
    Require-Path $sourceTrackFile "Route/runtime diagnostic track for $n64Name"
    ...
    $record = [ordered]@{
        ...
        route_context_class = [string]$routeRow.route_context_class
        route_trigger = [string]$routeRow.route_trigger
        capture_status = [string]$routeRow.capture_status
        route_acceptance_gate = [string]$routeRow.route_acceptance_gate
        exact_direct_player_actor_reference_count = [int]$routeRow.exact_direct_player_actor_reference_count
        family_route_reference_count = [int]$routeRow.family_route_reference_count
        ...
        runtime_acceptance_status = "route_runtime_diagnostic_package_candidate_not_runtime_accepted"
    }
...
$manifest = [ordered]@{
    format = "oot3d_link_child_route_runtime_diagnostic_track_batch_v1"
    base_track_batch_format = "oot3d_csab_skeleton_track_batch_v1"
    status = "route_runtime_diagnostic_track_batch_ready"
    policy = [ordered]@{
        scope = "Link child route/runtime-gated diagnostic derivatives"
        semantic_effect = "diagnostic package only; it does not accept routes, installed dumps, or mapping promotions"
        runtime_acceptance_policy = "rows remain diagnostic until exact route, accepted sibling route, or installed draw capture is accepted"
    }
    source_frontier_csv = $frontierCsv
    source_route_runtime_capture_matrix = $routeMatrixSummaryPath
...
Require-Path $archiveOutput "Link child route/runtime diagnostic package archive"
Require-Path $packageAuditOutput "Link child route/runtime diagnostic package audit"
...
$summary = [ordered]@{
    format = "oot3d_link_child_route_runtime_diagnostic_package_summary_v1"
    status = if ([int]$packageAudit.issue_counts.total -eq 0 -and $diagnosticBuildSummary.status -eq "route_runtime_diagnostic_tracks_ready") { "package_ready_pending_runtime_skinned_parity" } else { "package_invalid" }
    ...
    route_runtime_capture_matrix = $routeMatrixSummaryPath
    ...
    route_runtime_capture_counts = [ordered]@{
        record_count = $routeMatrixSummary.record_count
        exact_direct_player_actor_reference_count = $routeMatrixSummary.exact_direct_player_actor_reference_count
        family_route_present_count = $routeMatrixSummary.family_route_present_count
        accepted_route_count = $routeMatrixSummary.accepted_route_count
        installed_runtime_dump_valid_count = $routeMatrixSummary.installed_runtime_dump_valid_count
        issue_count = $routeMatrixSummary.issue_count
    }
    ...
    next_gate = "Run offline runtime/skinned pose parity, then prove exact route or installed runtime evidence before promotion."
}
$summary | ConvertTo-Json -Depth 12 | Set-Content -LiteralPath $summaryOutput -Encoding UTF8

if ($Verify) {
    Assert-Condition ($diagnosticBuildSummary.status -eq "route_runtime_diagnostic_tracks_ready") "expected diagnostic build ready"
    Assert-Condition ($diagnosticBuildSummary.route_runtime_frontier_row_count -eq 8) "expected eight route/runtime frontier rows"
    Assert-Condition ($summary.status -eq "package_ready_pending_runtime_skinned_parity") "expected ready package summary"
    Assert-Condition ($summary.exported -eq 8) "expected eight route/runtime tracks"
    Assert-Condition ($summary.failed -eq 0) "expected zero failed route/runtime tracks"
    Assert-Condition ($summary.route_runtime_capture_counts.record_count -eq 8) "expected eight route/runtime matrix records"
    Assert-Condition ($summary.route_runtime_capture_counts.exact_direct_player_actor_reference_count -eq 0) "expected zero exact route refs"
    Assert-Condition ($summary.route_runtime_capture_counts.family_route_present_count -eq 8) "expected family route evidence for all rows"
    Assert-Condition ($summary.route_runtime_capture_counts.accepted_route_count -eq 0) "expected zero accepted route/runtime routes"
    Assert-Condition ($summary.route_runtime_capture_counts.installed_runtime_dump_valid_count -eq 0) "expected zero accepted installed route/runtime dumps"
    Assert-Condition ($summary.package_audit_counts.archive_entry_count -eq 10) "expected 8 tracks plus 2 manifests"
    ...
    Assert-Condition ($summary.package_audit_counts.exported_record_count -eq 8) "expected eight exported package records"
    Assert-Condition ($summary.package_audit_counts.expected_track_count -eq 8) "expected eight package track entries"
    Assert-Condition ($summary.package_audit_counts.expected_unique_track_count -eq 8) "expected eight unique package track entries"
    Assert-Condition ($summary.package_audit_counts.track_entry_count -eq 8) "expected eight archived track entries"
...
Write-Host "OOT3D Link child route/runtime diagnostic package: $archiveOutput"
Write-Host "OOT3D Link child route/runtime diagnostic package summary: $summaryOutput"
