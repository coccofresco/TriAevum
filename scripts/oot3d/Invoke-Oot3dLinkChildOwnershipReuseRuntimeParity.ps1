...
if ([string]::IsNullOrWhiteSpace($PackageRoot)) {
    $PackageRoot = Join-Path $characterRoot "ownership_reuse_diagnostic_package"
}
if ([string]::IsNullOrWhiteSpace($OutputRoot)) {
    $OutputRoot = Join-Path $characterRoot "ownership_reuse_diagnostic_runtime_parity"
}
...
function Assert-Condition([bool]$Condition, [string]$Message) {
    if (-not $Condition) {
        throw "OOT3D Link child ownership-reuse runtime parity failed: $Message"
    }
}
...
Require-Path $PackageRoot "Ownership-reuse diagnostic package root"

$packageSummaryPath = Join-Path $PackageRoot "link_child_ownership_reuse_diagnostic_package_summary.json"
$packageAuditPath = Join-Path $PackageRoot "link_child_ownership_reuse_diagnostic_package_audit.json"
$trackBatchManifestPath = Join-Path $PackageRoot "link_child_ownership_reuse_diagnostic_track_batch_manifest.json"
$archivePath = Join-Path $PackageRoot "oot3d_link_child_ownership_reuse_diagnostic_candidates.o2r"
$summaryOutput = Join-Path $OutputRoot "link_child_ownership_reuse_diagnostic_runtime_parity_summary.json"
$auditRoot = Join-Path $OutputRoot "audits"
$glbRoot = Join-Path $OutputRoot "glb"

Require-Path $packageSummaryPath "Ownership-reuse diagnostic package summary"
Require-Path $packageAuditPath "Ownership-reuse diagnostic package audit"
Require-Path $trackBatchManifestPath "Ownership-reuse diagnostic track batch manifest"
Require-Path $archivePath "Ownership-reuse diagnostic package archive"
...
if ($Verify) {
    ...
    Assert-Condition ([int]$packageSummary.exported -eq 21) "expected 21 packaged ownership-reuse tracks"
    ...
    Assert-Condition ($trackBatchManifest.status -eq "ownership_reuse_diagnostic_track_batch_ready") "expected ownership-reuse track batch ready"
    Assert-Condition ([int]$trackBatchManifest.exported -eq 21) "expected 21 ownership-reuse track batch records"
...
foreach ($record in @($trackBatchManifest.records)) {
    ...
    Require-Path $trackPath "Packaged ownership-reuse track"
    ...
    Require-Path $auditOutput "Ownership-reuse runtime parity audit"
    Require-Path $runtimeGlbOutput "Ownership-reuse runtime GLB"
    ...
    $records += [ordered]@{
        ...
        policy_decision_scope = $record.policy_decision_scope
        ownership_reuse_arbitration_class = $record.ownership_reuse_arbitration_class
        source_group_class = $record.source_group_class
        ...
        runtime_acceptance_status = "offline_runtime_skinned_pose_parity_valid_pending_policy_and_installed_draw_capture"
    }
}
...
$summary = [ordered]@{
    format = "oot3d_link_child_ownership_reuse_diagnostic_runtime_parity_summary_v1"
    status = $status
    runtime_acceptance_status = if ($status -eq "offline_runtime_skinned_pose_parity_valid") {
        "offline_runtime_skinned_pose_parity_valid_pending_policy_and_installed_draw_capture"
    } else {
        "offline_runtime_skinned_pose_parity_invalid"
    }
    ...
    records = $records
    next_gate = "Make explicit ownership-reuse policy decisions, then capture/diff installed runtime draw output before promotion."
}
...
if ($Verify) {
    ...
    Assert-Condition ($summary.exported -eq 21) "expected twenty-one runtime parity records"
    Assert-Condition ($summary.failed -eq 0) "expected zero failed runtime parity records"
    Assert-Condition ($summary.counts.runtime_track_count -eq 21) "expected twenty-one runtime tracks"
    Assert-Condition ($summary.counts.valid_runtime_track_count -eq 21) "expected twenty-one valid runtime tracks"
    Assert-Condition ($summary.counts.runtime_primitive_count -eq 252) "expected 252 selected runtime primitives"
    Assert-Condition ($summary.counts.runtime_skinned_primitive_count -eq 210) "expected 210 selected skinned primitives"
    Assert-Condition ($summary.counts.runtime_rigid_primitive_count -eq 42) "expected 42 selected rigid primitives"
...
Write-Host "OOT3D Link child ownership-reuse runtime parity: $summaryOutput"
