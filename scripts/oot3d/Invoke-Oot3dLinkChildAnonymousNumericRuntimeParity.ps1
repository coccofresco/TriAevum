...
if ([string]::IsNullOrWhiteSpace($PackageRoot)) {
    $PackageRoot = Join-Path $characterRoot "anonymous_numeric_diagnostic_package"
}
if ([string]::IsNullOrWhiteSpace($OutputRoot)) {
    $OutputRoot = Join-Path $characterRoot "anonymous_numeric_diagnostic_runtime_parity"
}
...
function Assert-Condition([bool]$Condition, [string]$Message) {
    if (-not $Condition) {
        throw "OOT3D Link child anonymous numeric runtime parity failed: $Message"
    }
}
...
Require-Path $PackageRoot "Anonymous numeric diagnostic package root"

$packageSummaryPath = Join-Path $PackageRoot "link_child_anonymous_numeric_diagnostic_package_summary.json"
$packageAuditPath = Join-Path $PackageRoot "link_child_anonymous_numeric_diagnostic_package_audit.json"
$trackBatchManifestPath = Join-Path $PackageRoot "link_child_anonymous_numeric_diagnostic_track_batch_manifest.json"
$archivePath = Join-Path $PackageRoot "oot3d_link_child_anonymous_numeric_diagnostic_candidates.o2r"
$summaryOutput = Join-Path $OutputRoot "link_child_anonymous_numeric_diagnostic_runtime_parity_summary.json"
$auditRoot = Join-Path $OutputRoot "audits"
$glbRoot = Join-Path $OutputRoot "glb"

Require-Path $packageSummaryPath "Anonymous numeric diagnostic package summary"
Require-Path $packageAuditPath "Anonymous numeric diagnostic package audit"
Require-Path $trackBatchManifestPath "Anonymous numeric diagnostic track batch manifest"
Require-Path $archivePath "Anonymous numeric diagnostic package archive"
...
if ($Verify) {
    ...
    Assert-Condition ([int]$packageSummary.exported -eq 11) "expected 11 packaged anonymous numeric tracks"
    ...
    Assert-Condition ($trackBatchManifest.status -eq "anonymous_numeric_diagnostic_track_batch_ready") "expected anonymous numeric track batch ready"
    Assert-Condition ([int]$trackBatchManifest.exported -eq 11) "expected 11 anonymous numeric track batch records"
...
foreach ($record in @($trackBatchManifest.records)) {
    ...
    Require-Path $trackPath "Packaged anonymous numeric track"
    ...
    Require-Path $auditOutput "Anonymous numeric runtime parity audit"
    Require-Path $runtimeGlbOutput "Anonymous numeric runtime GLB"
    ...
    $records += [ordered]@{
        ...
        ownership_gate = $record.ownership_gate
        ...
        runtime_acceptance_status = "offline_runtime_skinned_pose_parity_valid_pending_identity_ownership_and_installed_draw_capture"
    }
}
...
$summary = [ordered]@{
    format = "oot3d_link_child_anonymous_numeric_diagnostic_runtime_parity_summary_v1"
    status = $status
    runtime_acceptance_status = if ($status -eq "offline_runtime_skinned_pose_parity_valid") {
        "offline_runtime_skinned_pose_parity_valid_pending_identity_ownership_and_installed_draw_capture"
    } else {
        "offline_runtime_skinned_pose_parity_invalid"
    }
    ...
    records = $records
    next_gate = "Resolve anonymous numeric identity and CSAB ownership, then capture/diff installed runtime draw output before promotion."
}
...
if ($Verify) {
    ...
    Assert-Condition ($summary.exported -eq 11) "expected eleven runtime parity records"
    Assert-Condition ($summary.failed -eq 0) "expected zero failed runtime parity records"
    Assert-Condition ($summary.counts.runtime_track_count -eq 11) "expected eleven runtime tracks"
    Assert-Condition ($summary.counts.valid_runtime_track_count -eq 11) "expected eleven valid runtime tracks"
    Assert-Condition ($summary.counts.runtime_primitive_count -eq 132) "expected 132 selected runtime primitives"
    Assert-Condition ($summary.counts.runtime_skinned_primitive_count -eq 110) "expected 110 selected skinned primitives"
    Assert-Condition ($summary.counts.runtime_rigid_primitive_count -eq 22) "expected 22 selected rigid primitives"
...
Write-Host "OOT3D Link child anonymous numeric runtime parity: $summaryOutput"
