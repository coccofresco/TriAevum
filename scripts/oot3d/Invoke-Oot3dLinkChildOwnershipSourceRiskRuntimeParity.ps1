...
if ([string]::IsNullOrWhiteSpace($PackageRoot)) {
    $PackageRoot = Join-Path $characterRoot "ownership_source_risk_diagnostic_package"
}
if ([string]::IsNullOrWhiteSpace($OutputRoot)) {
    $OutputRoot = Join-Path $characterRoot "ownership_source_risk_diagnostic_runtime_parity"
}
...
function Assert-Condition([bool]$Condition, [string]$Message) {
    if (-not $Condition) {
        throw "OOT3D Link child ownership source-risk runtime parity failed: $Message"
    }
}
...
Require-Path $PackageRoot "Ownership source-risk diagnostic package root"

$packageSummaryPath = Join-Path $PackageRoot "link_child_ownership_source_risk_diagnostic_package_summary.json"
$packageAuditPath = Join-Path $PackageRoot "link_child_ownership_source_risk_diagnostic_package_audit.json"
$trackBatchManifestPath = Join-Path $PackageRoot "link_child_ownership_source_risk_diagnostic_track_batch_manifest.json"
$archivePath = Join-Path $PackageRoot "oot3d_link_child_ownership_source_risk_diagnostic_candidates.o2r"
$summaryOutput = Join-Path $OutputRoot "link_child_ownership_source_risk_diagnostic_runtime_parity_summary.json"
$auditRoot = Join-Path $OutputRoot "audits"
$glbRoot = Join-Path $OutputRoot "glb"

Require-Path $packageSummaryPath "Ownership source-risk diagnostic package summary"
Require-Path $packageAuditPath "Ownership source-risk diagnostic package audit"
Require-Path $trackBatchManifestPath "Ownership source-risk diagnostic track batch manifest"
Require-Path $archivePath "Ownership source-risk diagnostic package archive"
...
if ($Verify) {
    ...
    Assert-Condition ([int]$packageSummary.exported -eq 2) "expected 2 packaged source-risk tracks"
    ...
    Assert-Condition ($trackBatchManifest.status -eq "ownership_source_risk_diagnostic_track_batch_ready") "expected source-risk track batch ready"
    Assert-Condition ([int]$trackBatchManifest.exported -eq 2) "expected 2 source-risk track batch records"
...
foreach ($record in @($trackBatchManifest.records)) {
    ...
    Require-Path $trackPath "Packaged ownership source-risk track"
    ...
    Require-Path $auditOutput "Ownership source-risk runtime parity audit"
    Require-Path $runtimeGlbOutput "Ownership source-risk runtime GLB"
    ...
    $records += [ordered]@{
        ...
        derivative_frontier_class = $record.derivative_frontier_class
        reference_envelope_status = $record.reference_envelope_status
        normalized_extent_max_abs_delta_max = $record.normalized_extent_max_abs_delta_max
        center_delta_normalized_max = $record.center_delta_normalized_max
        scale_factor_oot3d_per_n64_avg = $record.scale_factor_oot3d_per_n64_avg
        ...
        runtime_acceptance_status = "offline_runtime_skinned_pose_parity_valid_pending_alternate_source_or_installed_draw_capture"
    }
}
...
$summary = [ordered]@{
    format = "oot3d_link_child_ownership_source_risk_diagnostic_runtime_parity_summary_v1"
    status = $status
    runtime_acceptance_status = if ($status -eq "offline_runtime_skinned_pose_parity_valid") {
        "offline_runtime_skinned_pose_parity_valid_pending_alternate_source_or_installed_draw_capture"
    } else {
        "offline_runtime_skinned_pose_parity_invalid"
    }
    ...
    records = $records
    next_gate = "Review alternate source candidates or capture/diff installed runtime draw output before promotion."
}
...
if ($Verify) {
    ...
    Assert-Condition ($summary.exported -eq 2) "expected two runtime parity records"
    Assert-Condition ($summary.failed -eq 0) "expected zero failed runtime parity records"
    Assert-Condition ($summary.counts.runtime_track_count -eq 2) "expected two runtime tracks"
    Assert-Condition ($summary.counts.valid_runtime_track_count -eq 2) "expected two valid runtime tracks"
    Assert-Condition ($summary.counts.runtime_primitive_count -eq 24) "expected 24 selected runtime primitives"
    Assert-Condition ($summary.counts.runtime_skinned_primitive_count -eq 20) "expected 20 selected skinned primitives"
    Assert-Condition ($summary.counts.runtime_rigid_primitive_count -eq 4) "expected 4 selected rigid primitives"
...
Write-Host "OOT3D Link child ownership source-risk runtime parity: $summaryOutput"
